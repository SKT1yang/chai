...
export function createCancelablePromise<T>(callback: (token: CancellationToken) => Promise<T>): CancelablePromise<T> {
	...
	return <CancelablePromise<T>>new (class {
		...
		then<
			TResult1 = T,
			TResult2 = never,
		>(resolve?: ((value: T) => TResult1 | Promise<TResult1>) | null, reject?: ((reason: unknown) => TResult2 | Promise<TResult2>) | null): Promise<TResult1 | TResult2> {
			return promise.then(resolve, reject);
		}
		catch<
			TResult = never,
		>(reject?: ((reason: unknown) => TResult | Promise<TResult>) | null): Promise<T | TResult> {
...
export class Delayer<T> implements IDisposable {
	...
	private doResolve: ((value?: any) => void) | null;
...
export class TaskQueue {
	...
	private async _processQueue(): Promise<void> {
		...
		try {
			...
			void next.deferred.complete(result);
		} catch (e) {
			void next.deferred.error(e);
		} finally {
	...
	public clearPending(): void {
		...
		for (const task of tasks) {
			if (task.setUndefinedWhenCleared) {
				void task.deferred.complete(undefined);
			} else {
				void task.deferred.error(new CancellationError());
			}
		}
	}
}
...
export class TimeoutTimer implements IDisposable {
	private _token: NodeJS.Timeout | undefined;
	private _isDisposed = false;
...
export class RunOnceScheduler implements IDisposable {
	...
	private timeoutToken: NodeJS.Timeout | undefined;
...
export class DeferredPromise<T> {
	public static fromPromise<T>(promise: Promise<T>): DeferredPromise<T> {
		const deferred = new DeferredPromise<T>();
		void deferred.settleWith(promise);
		return deferred;
	}
...
class ProducerConsumer<T> {
	...
	private _resolveOrRejectDeferred(deferred: DeferredPromise<T>, value: ProducerConsumerValue<T>): void {
		if (value.ok) {
			void deferred.complete(value.value);
		} else {
			void deferred.error(value.error);
		}
	}
