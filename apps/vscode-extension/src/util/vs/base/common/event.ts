...
export namespace Event {
	...
	export function debounce<I, O>(
		...
		let handle: NodeJS.Timeout | undefined | null = undefined;
				...
				subscription = event((cur) => {
					...
					if (handle === undefined) {
						...
						if (typeof delay === 'number') {
							...
							handle = 0 as unknown as NodeJS.Timeout;
							queueMicrotask(() => {
								// Fire on trailing edge if there were calls during throttle period
								if (trailing && numThrottledCalls > 0) {
									emitter.fire(output!);
	...
	class ChainableSynthesis implements IChainableSythensis<any> {
		...
		reduce<R>(merge: (last: R | undefined, event: any) => R, initial?: R): this {
			let last = initial;
			this.steps.push((v) => {
				last = merge(last, v);
				return last;
	...
	export function toPromise<T>(event: Event<T>, disposables?: IDisposable[] | DisposableStore): CancelablePromise<T> {
		...
		if (disposables) {
			void promise.finally(() => disposeAndRemove(listener, disposables));
		}
...
export class Relay<T> implements IDisposable {
	...
	private readonly emitter = new Emitter<T>({
		onDidAddFirstListener: () => {
			this.listening = true;
			this.inputEventListener = this.inputEvent((e) => this.emitter.fire(e));
		},
		onDidRemoveLastListener: () => {
