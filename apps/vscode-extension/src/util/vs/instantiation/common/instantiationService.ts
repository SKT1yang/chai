...
export class InstantiationService implements IInstantiationService {
		...
		try {
			const accessor: ServicesAccessor = {
				get: <T>(id: ServiceIdentifier<T>) => {
					...
					if (!result) {
						this._throwIfStrict(`[invokeFunction] unknown service '${id.toString()}'`, false)
					}
					return result
				},
			}
	...
	createInstance(ctorOrDescriptor: any, ...rest: unknown[]): unknown {
		this._throwIfDisposed()
	...
	private _createInstance<T>(ctor: any, args: unknown[] = [], _trace: Trace): T {
		...
		for (const dependency of serviceDependencies) {
			...
			if (!service) {
				this._throwIfStrict(
					`[createInstance] ${ctor.name} depends on UNKNOWN service ${dependency.id.toString()}.`,
					false,
				)
			}
	...
	private _safeCreateAndCacheServiceInstance<T>(id: ServiceIdentifier<T>, desc: SyncDescriptor<T>, _trace: Trace): T {
		if (this._activeInstantiations.has(id)) {
			throw new Error(`illegal state - RECURSIVELY instantiating service '${id.toString()}'`)
		}
	...
	private _createAndCacheServiceInstance<T>(id: ServiceIdentifier<T>, desc: SyncDescriptor<T>, _trace: Trace): T {
		...
		while (stack.length) {
			...
			for (const dependency of _util.getServiceDependencies(item.desc.ctor)) {
				...
				if (!instanceOrDesc) {
					this._throwIfStrict(
						`[createInstance] ${id.toString()} depends on ${dependency.id.toString()} which is NOT registered.`,
						true,
					)
				}
...
export class Trace {
	...
	stop() {
		...
		function printChild(n: number, trace: Trace) {
			const res: string[] = []
			const prefix = [n + 1].join('\t')
			for (const [id, first, child] of trace._dep) {
				if (first && child) {
					causedCreation = true
					res.push(`${prefix}CREATES -> ${id.toString()}`)
					const nested = printChild(n + 1, child)
					if (nested) {
						res.push(nested)
					}
				} else {
					res.push(`${prefix}uses -> ${id.toString()}`)
				}
			}
			return res.join('\n')
		}
